...
import java.net.URLEncoder.{encode=>jencode}

class QueryParameters(private val m: Map[String,String]) {
  /** Add two sets of query parameters together just involves adding together the two maps. */
...
object QueryParameters {
  ...
  def apply(mappings: (String,String)*): QueryParameters = apply(Map(mappings: _*))
}
